...
/*void multiply(float* input, float* output, float* m, int attrCount, int tCount, float* tValues) {
...
/*__kernel void drawWireframeDots(
		__global float* input, int attrCount, int tCount) {
	int i = get_global_id(0);
	for (int j = i; j < tCount; j += get_global_size(0)) {
		printf("%d\n", j);
	}
	//printf("%d\n", i);
}*/
